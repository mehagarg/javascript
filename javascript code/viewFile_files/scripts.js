...
function onLoad() {
  ...
  if (isMobile.iOS()||isMobile.Android()) {
    // NOTE: (ak) this if is just to accomodate FileSharing beta testing using the production environment
    // it's only here because 4.1.1 and bellow Android tablet does not know the shareLink URL schema, so we need to download directly if this is and Adnroid tablet client
    // once the Android 5.0.0 is on the store, we can remove the if {} and preserve only the else {}
    if (isMobile.Android()&&(!navigator.userAgent.match(/Mobile/))) {
      // download the file directly, just like the old flow
      document.location = apiUrl + "/downloadSharedLink?linkId=" + window.location.getParameter("linkId");
    }
    else {
    // if the app is not installed or does not respond to the custom URL schema
    // redirect in the browser to the laning page after 2 seconds
    setTimeout(function () { redirectToCDN(apiUrl,type,linkId); }, 2000);
    // redirect to the custom URL screma
